...
public class AVL<T extends Comparable<T>> extends BST {
    ...
    public Node<T> leftMost(Node<T> cur) {
        Node<T> current = cur;
    ...
    public Node<T> deleteNode(Node<T> cur, T x) {
        ...
        if (cmp < 0) {
            ...
            if (cur.isLeaf()) cur = null;
            else if (cur.hasOneChild()) cur = child;
            else {
                Node<T> leftMostNode = leftMost(cur.right);
                cur.data = leftMostNode.data;
                cur.right = deleteNode(cur.right, leftMostNode.data);
            }
        }

...
FPNode::FPNode(const Item &item, const std::shared_ptr<FPNode> &parent) : item(item), frequency(1), node_link(nullptr), parent(parent), children() {
}

FPTree::FPTree(const std::vector<Transaction> &transactions, uint64_t minSup) : root(std::make_shared<FPNode>(Item{}, nullptr)), header_table(),
    minSup(minSup) {
    ...
    for (auto it = frequency_by_item.cbegin(); it != frequency_by_item.cend();) {
        const uint64_t itemFreq = (*it).second;
        if (itemFreq < minSup) {
    ...
    std::set<std::pair<Item, uint64_t>, cmpFreq> items_ordered_by_frequency(frequency_by_item.cbegin(), frequency_by_item.cend());
...
std::set<Pattern> fptree_growth(const FPTree &fptree) {
    ...
    if (contains_single_path(fptree)) {
        ...
        for (const auto &pair : fptree.header_table) {
            ...
            const FPTree conditional_fptree(conditional_fptree_transactions, fptree.minSup);
